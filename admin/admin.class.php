...
class admin {
	...
	function move_admin($username) {
		...
		if($r && $r['admin'] > 0) {			
			if($r['userid'] == $this->founderid) return $this->_('創始人不可改變級別');
			if($r['admin'] == 1 && $this->count_admin() < 2) return $this->_('系統最少需要保留一位超級管理員');
			$admin = $r['admin'] == 1 ? 2 : 1;
			$this->db->query("UPDATE {$this->pre}member SET admin=$admin WHERE username='$username'");
			return true;
		} else {
			return $this->_('管理員不存在');
		}
	}
	...
	function delete_admin($username) {
		$r = $this->get_one($username);
		if($r) {
			if($r['userid'] == $this->founderid) return $this->_('創始人不可刪除');
			if($r['admin'] == 1 && $this->count_admin() < 2) return $this->_('系統最少需要保留一位超級管理員');
			$userid = $r['userid'];
	...
	function get_list($condition) {
		...
		while($r = $this->db->fetch_array($result)) {
			...
			$r['adminname'] = $r['admin'] == 1 ? ($CFG['founderid'] == $r['userid'] ? '<span class="f_red">網站創始人</span>' : '<span class="f_blue">超級管理員</span>') : '普通管理員';
			$admins[] = $r;
		}
		return $admins;
	}
