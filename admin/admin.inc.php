...
$do = new admin;
$menus = array (
    array('添加管理員', '?moduleid='.$moduleid.'&file='.$file.'&action=add'),
    array('管理員管理', '?moduleid='.$moduleid.'&file='.$file),
    array('權限&面板', '?moduleid='.$moduleid.'&file='.$file.'&action=right&userid='.$_userid),
);
...
switch($action) {
	case 'add':
		if($submit) {
			...
			if($do->set_admin($username, $admin, $role, $aid)) {
				...
				msg('管理員添加成功，下一步請分配權限和管理面板', '?file='.$file.'&action=right&userid='.$userid);
			}
			msg($do->errmsg);
		} else {
	...
	break;
	case 'delete':
		if($do->delete_admin($username)) dmsg('撤銷成功', $this_forward);
		msg($do->errmsg);
	break;
	case 'right':
		if(!$userid) msg();
	...
	case 'ajax':
		...
		if(isset($fi)) {
			...
			if(isset($RT)) {
				$file_select = '<select name="right[0][file]" size="2" style="height:200px;width:150px;" onchange="get_action(this.value, '.$mid.');"><option value="">選擇文件[單選]</option>';
				...
				if($CT) {
					...
					echo '<option>選擇分類多選[按Ctrl鍵多選]</option>';
