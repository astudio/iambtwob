...
$menus = array (
    array('地區添加', '?file='.$file.'&action=add'),
    array('地區管理', '?file='.$file),
	array('導入省市', '?file='.$file.'&action=import','onclick="return confirm(\'確定導入中國省市數據嗎？ 當前數據將被覆蓋 \');"'),
...
switch($action) {
	...
	case 'import':
		...
		is_file($file) or msg('數據文件不存在，請上傳程序包內 file/setting/area.sql 文件至 file/setting 目錄');
		require DT_ROOT.'/include/sql.func.php';
		sql_execute(file_get($file));
		cache_area();
		dmsg('導入成功', $this_forward);
	break;
	case 'cache':
		$do->repair();
