...
switch($action) {
	case 'add':
		if($submit) {
			if(!$category['catname']) msg('分類名不能為空');
	...
	case 'edit':
		...
		if($submit) {
			if(!$category['catname']) msg('分類名不能為空');
			if($category['parentid'] == $catid) msg('上級分類不能與當前分類相同');
			$do->edit($category);
	...
	case 'copy':
		if($submit) {
			...
			$do->repair();
			msg('分類複製成功', '?file='.$file.'&action=url&&mid='.$mid.'&forward='.urlencode('?file='.$file.'&mid='.$mid));
		} else {
	...
	case 'count':
		...
		msg('ID從'.$fid.'至'.($catid-1).'更新成功'.progress($sid, $fid, $tid), "?mid=$mid&file=$file&action=$action&sid=$sid&fid=$catid&tid=$tid&num=$num");
	break;
	case 'ckdir':
		if($do->get_catdir($catdir)) {
			dialog('目錄名可以使用');
		} else {
			dialog('目錄名不合法或者已經被使用');
		}
	break;
	case 'url':	
	...
	default:
		...
		if(!$DTCAT && !$parentid) msg('暫無分類,請先添加',  '?file='.$file.'&mid='.$mid.'&action=add&parentid='.$parentid);
