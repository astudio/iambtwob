...
$menus = array (
    array('數據導入', '?file='.$file),
    array('數據備份', '?file=database'),
);
$this_forward = '?file='.$file;
isset($name) or $name = '';
if($name && !preg_match("/^[0-9a-z_\-\.]+$/i", $name)) msg('不是一個有效的文件名');
...
switch($action) {
	case 'config':
		if($submit) {
			...
			if($type == 0 && $mid == 0) msg('請選擇模型');
			if($type == 2 && $tb == '') msg('請選擇表');
			if($type == 0) {
				$sc_table = get_table($mid);
				$names = parse_dict(substr($sc_table, strlen($DT_PRE)));
				$names['content'] = $MODULE[$mid]['name'].'內容';
	...
	case 'import':
		...
		if($fid <= $tid) {
			...
			msg('轉換成功,成功導入 <strong>'.$total.'</strong> 條數據', "?file=$file", 10);
		}
		msg('ID從'.$fid.'至'.($keyid-1).'轉換成功 當前已導入 <strong>'.$total.'</strong> 條數據', "?file=$file&action=$action&name=$name&fid=$keyid&tid=$tid&num=$num&total=$total");
	break;
	default:
