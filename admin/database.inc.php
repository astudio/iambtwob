...
$menus = array (
    array('數據庫備份', '?file='.$file),
    array('數據庫恢復', '?file='.$file.'&action=import'),
    array('字符替換', '?file='.$file.'&action=replace'),
    array('執行SQL', '?file='.$file.'&action=execute'),
    array('顯示進程', '?file='.$file.'&action=process'),
    array('數據互轉', '?file='.$file.'&action=move'),
    array('數據導入', '?file=data'),
);
...
switch($action) {
	...
	break;
	case 'execute':
		if(!isset($CFG['executesql']) || !$CFG['executesql']) msg('系統禁止了執行SQL，請FTP修改根目錄config.inc.php<br/>$CFG[\'executesql\'] = \'0\'; 修改為 $CFG[\'executesql\'] = \'1\';');
		if($submit) {
			if(trim($sql) == '') {
				msg('SQL語句為空');
			} else {
				$sql = stripslashes($sql);
				if(preg_match("/DROP(.*)(TABLE|DATABASE)/i", $sql)) msg('系統禁止DROP語句');				
				sql_execute($sql);
				dmsg('執行成功', '?file='.$file.'&action=execute');
			}
		} else {
	...
	case 'kill':
		...
		dmsg('結束成功', '?file='.$file.'&action=process');
	break;
	case 'comments':
		$db->halt = 0;
		...
		foreach($MODULE as $k=>$v) {
			if(in_array($v['module'], array('article', 'info'))) {
				...
				$db->query($sql);
				$sql = "ALTER TABLE `".$DT_PRE.$v['module']."_data_".$v['moduleid']."` COMMENT='".$v['name']."內容'";
				$db->query($sql);
			}
		}
		dmsg('重建成功', '?file='.$file);
	...
	case 'move':
		if($submit) {
			...
			message('成功轉移 '.$i.' 條數據', '?file='.$file.'&action='.$action, 2);
		} else {
	...
	case 'replace':
		if($submit) {
			...
			$key or msg('表'.$table.'無主鍵，無法完成操作');
			$key != $fields or msg('無法完成主鍵操作');
	...
	break;
	case 'file_replace':
		if(!$file_pre) msg('請選擇或者填寫備份文件前綴');
		if(!$file_from) msg('請請填寫查找內容');
		...
		if(is_file($dfile)) {
			...
			file_put($dfile, $sql);
			$fid = $fileid;
			msg('分卷 <strong>#'.$fileid++.'</strong> 替換成功 程序將自動繼續...'.progress(0, $fid, $tid), '?file='.$file.'&action='.$action.'&file_pre='.$file_pre.'&fileid='.$fileid.'&tid='.$tid.'&file_from='.urlencode($file_from).'&file_to='.urlencode($file_to));
		} else {
			msg('文件內容替換成功', '?file='.$file.'&action=replace');
		}
	break;
	case 'open':
		if(!$dir) msg('請選擇備份系列');
		if(!is_dir($D.$dir)) msg('備份系列不存在');
		$sql = $sqls = array();
		$sqlfiles = glob($D.$dir.'/*.sql');
		if(!$sqlfiles) msg('備份系列文件不存在');
		$tid = count($sqlfiles);
		foreach($sqlfiles as $id=>$sqlfile)	{
			$tmp = basename($sqlfile);
			$sql['filename'] = $tmp;
	...
	case 'fields':
		...
		echo '<select name="fields" id="fd"><option value="">選擇字段</option>'.$fields_select.'</select>';
		exit;
	break;
	case 'import':
		if(isset($import)) {
			if(isset($filename) && $filename && file_ext($filename) == 'sql') {
				$dfile = $D.$filename;
				if(!is_file($dfile)) msg('文件不存在，請檢查');
				$sql = file_get($dfile);
				sql_execute($sql);
				msg($filename.' 導入成功', '?file='.$file.'&action=import');
			} else {
				$fileid = isset($fileid) ? $fileid : 1;
				...
				if(is_file($filename)) {
					...
					if(substr($sql, 0, 11) == '# Destoon V') {
						$v = substr($sql, 11, 3);
						if(DT_VERSION != $v) msg('由於數據結構存在差異，備份數據不可以跨版本導入<br/>備份版本：V'.$v.'<br/>當前系統：V'.DT_VERSION);
					}
					sql_execute($sql);
					$prog = $tid ? progress(1, $fileid, $tid) : '';
					msg('分卷 <strong>#'.$fileid++.'</strong> 導入成功 程序將自動繼續...'.$prog, '?file='.$file.'&action='.$action.'&filepre='.$filepre.'&fileid='.$fileid.'&tid='.$tid.'&import=1');
				} else {
					msg('數據庫恢復成功', '?file='.$file.'&action=import');
				}
			}
		} else {
	...
	default:
		if(isset($backup)) {
			...
			if($fileid == 1 && $tables) {
				if(!isset($tables) || !is_array($tables)) msg('請選擇需要備份的表');
				...
				if(!$tables = cache_read($_username.'_backup.php')) msg('請選擇需要備份的表');
			}
			...
			if(trim($sqldump)) {
				...
				$fid = $fileid;
				msg('分卷 <strong>#'.$fileid++.'</strong> 備份成功.. 程序將自動繼續...'.progress(0, $fid, $tid), '?file='.$file.'&sizelimit='.$sizelimit.'&sqlcompat='.$sqlcompat.'&sqlcharset='.$sqlcharset.'&tableid='.$tableid.'&fileid='.$fileid.'&fileid='.$fileid.'&tid='.$tid.'&startfrom='.$startrow.'&random='.$random.'&backup=1');
			} else {
			   ...
			   msg('數據庫備份成功', '?file='.$file.'&action=import');
			}
		} else {
