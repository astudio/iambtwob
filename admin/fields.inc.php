...
class fields {
	...
	function pass($post) {
		...
		if(!$post['name']) return $this->_('請填寫字段');
		if(!preg_match("/^[a-z0-9]+$/", $post['name'])) return $this->_('字段名只能為小寫字母和數字的組合');
		if(!$post['title']) return $this->_('請填寫字段名稱');
		if(in_array($post['html'], array('select', 'radio', 'checkbox'))) {
			if(!$post['option_value']) return $this->_('請填寫選項值');
			if(strpos($post['option_value'], '|') === false) return $this->_('請填寫正確的選項值');
		}
		return true;
	}
