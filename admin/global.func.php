...
function admin_log($force = 0) {
	...
	if($DT['admin_log'] == 2 || ($DT['admin_log'] == 1 && ($file == 'setting' || in_array($action, array('delete', 'edit', 'move', 'clear', 'add'))))) {
		...
		$db->query("INSERT INTO {$db->pre}admin_log(qstring, username, ip, logtime) VALUES('$qstring','$_username','$DT_IP','$DT_TIME')");
...
function split_content($moduleid, $part) {
	...
	$db->query("CREATE TABLE IF NOT EXISTS `{$table}` (`{$fd}` bigint(20) unsigned NOT NULL default '0',`content` longtext NOT NULL,PRIMARY KEY  (`{$fd}`))".$type." COMMENT='".$MODULE[$moduleid]['name']."內容_".$part."'");
}

function split_sell($part) {
	global $db, $CFG, $MODULE;
	$sql = file_get(DT_ROOT.'/file/setting/split_sell.sql');
	$sql or dalert('請檢查文件file/setting/split_sell.sql是否存在');
...
function seo_title($title, $show = '') {
	$SEO = array(
		...
		'sitename'			=>	'網站名稱',
		'sitetitle'			=>	'網站SEO標題',
		'sitekeywords'		=>	'網站SEO關鍵詞',
		'sitedescription'	=>	'網站SEO描述',
		'catname'			=>	'分類名稱',
		'cattitle'			=>	'分類SEO標題',
		'catkeywords'		=>	'分類SEO關鍵詞',
		'catdescription'	=>	'分類SEO描述',
		'showtitle'			=>	'內容標題',
		'showintroduce'		=>	'內容簡介',
		'kw'				=>	'關鍵詞',
		'areaname'			=>	'地區',
