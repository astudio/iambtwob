...
switch($action) {
	case 'cache':
		...
		cache_clear('area');
		cache_area();
		msg('緩存更新成功', '?file='.$file.'&action=module');
	break;
	case 'all':
		msg('全站更新成功');
	break;
	case 'index':
		tohtml('index');
		msg('網站首頁生成成功', '?file='.$file.'&action=all');
	break;
	case 'back':
		$moduleids = 0;
		unset($MODULE[1]);
	...
	case 'module':
		if(isset($moduleids)) {
			...
			if(isset($KEYS[$moduleids])) {
				...
				msg('模塊更新成功', '?file='.$file.'&action=index');
			}		
		} else {
			$moduleids = 0;
			msg('開始更新模塊', '?file='.$file.'&action='.$action.'&moduleids='.$moduleids);
		}
	break;
	default:
		msg('正在開始更新全站', '?file='.$file.'&action=cache');
	break;
}
?>