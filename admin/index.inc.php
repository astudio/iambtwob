...
$menus = array (
array('系統首頁', '?action=main'),
array('統計信息', '?action=count'),
array('更新緩存', '?action=cache'),
array('修改密碼', '?action=password'),
array('商務中心', $MODULE[2]['linkurl'], 'target="_blank"'),
array('網站首頁', DT_PATH, 'target="_blank"'),
array('安全退出', '?file=logout','target="_top" onclick="return confirm(\'確定要退出管理嗎?\');"'),
);
...
switch($action) {
	case 'cache':
		...
		if($step == 1) {
			cache_clear('module');
			cache_module();
			msg('系統設置更新成功', '?action='.$action.'&step='.($step+1));
		} else if($step == 2) {
			cache_clear_tag(1);
			msg('標籤調用緩存更新成功', '?action='.$action.'&step='.($step+1));
		} else if($step == 3) {
			cache_clear('php', 'dir', 'tpl');
			msg('模板緩存更新成功', '?action='.$action.'&step='.($step+1));
		} else if($step == 4) {
			cache_clear('cat');
			cache_category();
			msg('分類緩存更新成功', '?action='.$action.'&step='.($step+1));
		} else if($step == 5) {
			cache_clear('area');
			cache_area();
			msg('地區緩存更新成功', '?action='.$action.'&step='.($step+1));
		} else if($step == 6) {
			cache_clear('fields');
			cache_fields();
			cache_clear('option');
			if(isset($MODULE[7])) cache_quote_product();
			msg('自定義字段更新成功', '?action='.$action.'&step='.($step+1));
		} else if($step == 7) {
			cache_clear_ad();
			tohtml('index');
			msg('全部緩存更新成功');
			...
			cache_banword();
			cache_bancomment();
			msg('正在開始更新緩存', '?action='.$action.'&step='.($step+1));
		}
	break;
	case 'cacheclear':
	...
	case 'update':
		...
		if(isset($dir)) {
				...
				if($dirs) {
					...
					if($files) {
						...
						foreach($files as $f) {
							...
							$cf++;
							$i++;
							if($i > 500) msg('已刪除 '.$cd.' 個目錄，'.$cf.' 個文件'.progress(0, $cd, $tt), '?action='.$action.'&dir='.$dir.'&cd='.$cd.'&cf='.$cf.'&job='.$job.'&tt='.$tt, 0);
						}
						dir_delete($sub);
						$cd++;
						msg('已刪除 '.$cd.' 個目錄，'.$cf.' 個文件'.progress(0, $cd, $tt), '?action='.$action.'&dir='.$dir.'&cd='.$cd.'&cf='.$cf.'&job='.$job.'&tt='.$tt, 0);
					} else {
						dir_delete($sub);
						$cd++;
						msg('已刪除 '.$cd.' 個目錄，'.$cf.' 個文件'.progress(0, $cd, $tt), '?action='.$action.'&dir='.$dir.'&cd='.$cd.'&cf='.$cf.'&job='.$job.'&tt='.$tt, 0);
			...
			if(rename(DT_CACHE.'/'.$job, DT_CACHE.'/'.$dir)) {
				...
				$dirs = glob(DT_CACHE.'/'.$dir.'/*');
				$tt = count($dirs);
				msg('正在更新，此操作可能用時較長，請不要中斷..', '?action='.$action.'&dir='.$dir.'&job='.$job.'&tt='.$tt);
			} else {
				msg('更新失敗');
			}
		}
	...
	case 'html':
		...
		msg('首頁更新成功 '.(is_file($filename) ? dround(filesize($filename)/1024).'Kb ' : '').'&nbsp;&nbsp;<a href="'.DT_PATH.'" target="_blank">點擊查看</a>');
	break;
	case 'phpinfo':
		phpinfo();
		exit;
	...
	case 'password':
		if($submit) {
			if(!$oldpassword) msg('請輸入現有密碼');
			if(!$password) msg('請輸入新密碼');
			if(strlen($password) < 6) msg('新密碼最少6位，請修改');
			if($password != $cpassword) msg('兩次輸入的密碼不一致，請檢查');
			$r = $db->get_one("SELECT password FROM {$DT_PRE}member WHERE userid='$_userid'");
			if($r['password'] != md5(md5($oldpassword)))  msg('現有密碼錯誤，請檢查');
			if($password == $oldpassword) msg('新密碼不能與現有密碼相同');
			$password = md5(md5($password));
			$db->query("UPDATE {$DT_PRE}member SET password='$password' WHERE userid='$_userid'");
			msg('管理員密碼修改成功', '?action=main');
		} else {
