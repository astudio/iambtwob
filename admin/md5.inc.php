...
$menus = array (
    ...
    array('文件校驗', '?file='.$file),
);
$sys = array('admin', 'api', 'include', 'javascript', 'lang', 'module', 'template', 'wap');
$fbs = array('file', 'cache');
switch($action) {
	case 'delete':
		preg_match("/^[0-9]{4}[\-]{1}[0-9]{2}[\-]{1}[0-9]{2}[\s]{1}[0-9]{2}[\.]{1}[0-9]{2}$/", $mirror) or msg('請選擇鏡像文件');
	...
	default:
		if($submit) {
			...
			$mirror = $mirror.'.php';
			is_file(DT_ROOT.'/file/md5/'.$mirror) or msg('請選擇鏡像文件');
			$filedir or $filedir = $sys;
			$fileext or $fileext = 'php|js|htm';
			$files = array();
			foreach(glob(DT_ROOT.'/*.*') as $f) {
			...
			is_file(DT_ROOT.'/file/md5/'.DT_VERSION.'.php') or msg('正在創建鏡像文件..', '?file='.$file.'&action=add');
			$files = glob(DT_ROOT.'/*');
			$dirs = $rfiles = array();
			foreach($files as $f) {
				if(is_file($f)) {
