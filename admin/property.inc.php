...
$CAT or msg('請指定分類ID');
$menus = array (
    array('添加屬性', '?file='.$file.'&catid='.$catid.'&action=add'),
    array('屬性參數', '?file='.$file.'&catid='.$catid),
);
$TYPE = array('單行文本(text)', '多行文本(textarea)', '列表選擇(select)', '復選框(checkbox)');
$do = new property;
$do->catid = $catid;
switch($action) {
	case 'add':
	...
	case 'delete':
		...
		$do->oid = $oid;
		$do->delete($pid);
		dmsg('刪除成功', '?file='.$file.'&catid='.$catid);
	break;
	default:
		$lists = $do->get_list();
...
class property {
	...
	function pass($post) {
		...
		if(!$post['name']) return $this->_('請填寫屬性名稱');
		if($post['type'] == 3) {
			if(!$post['value']) return $this->_('請填寫備選值');
			if(strpos($post['value'], '|') === false) return $this->_('最少需要設定2個備選值');
		}
		return true;
	}
