...
if($action == 'merge') {
	...
	if($fid <= $tid) {
		...
		msg($MODULE[$mid]['name'].'內容合併成功');
	}
	msg('ID從'.$fid.'至'.($itemid-1).'合併成功'.progress($sid, $fid, $tid), "?mid=$mid&file=$file&action=$action&sid=$sid&fid=$itemid&tid=$tid&num=$num");
} else {
	if(!isset($num)) {
		$num = 5000;
	}
	...
	if($fid <= $tid) {
		...
		msg($MODULE[$mid]['name'].'內容拆分成功');
	}
	msg('ID從'.$fid.'至'.($itemid-1).'拆分成功'.progress($sid, $fid, $tid), "?mid=$mid&file=$file&action=$action&sid=$sid&fid=$itemid&tid=$tid&num=$num");
}
?>