...
$menus = array (
    ...
    array('重建緩存', '?file='.$file.'&action=cache'),
...
switch($action) {
	case 'add':
		if($submit) {
			if(!preg_match("/^[a-z0-9_\-]+$/", $fileid)) msg('文件名只能為小寫字母、數字、中劃線、下劃線');
			if(!$name) $name = $fileid;
			$template = $template_root.'/'.$fileid.'.htm';
			if(isset($nowrite) && is_file($template)) msg('文件已經存在');
			file_put($template, stripslashes($content));
			if($name != $fileid) template_name($fileid, $name);
			dmsg('創建成功', $this_forward);
		} else {
			$content = '';
	...
	case 'edit':
		if($submit) {
			...
			$fileid or msg();
			if(!is_write($template_root.'/'.$fileid.'.htm')) msg($fileid.'.htm不可寫，請將其屬性設置為可寫');
	...
	case 'import':
		$fileid or msg();
		$bakid or msg();
		if(file_copy($template_root.'/'.$fileid.'.'.$bakid.'.bak', $template_root.'/'.$fileid.'.htm')) dmsg('恢復成功', $this_forward);
		msg('備份文件恢復失敗');
	break;
	case 'template_name':
		$fileid or exit('0');
	...
	default:
		...
		if(!$files) msg('模板文件不存在，請先創建', "?file=$file&action=add&dir=$dir");
