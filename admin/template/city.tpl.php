...
<tr>
<td>&nbsp;
<input type="text" size="50" name="kw" value="<?php echo $kw;?>" title="分站名稱或綁定域名"/>
&nbsp;
...
<a href="?file=<?php echo $file;?>&action=delete&areaid=<?php echo $v['areaid'];?>" onclick="return _delete();"><img src="admin/image/delete.png" width="16" height="16" title="刪除" alt=""/></a></td>
</tr>
<?php }?>
</table>
<div class="btns">&nbsp;
<input type="submit" name="submit" value="更新分站" class="btn" onclick="this.form.action='?file=<?php echo $file;?>&action=update'"/>&nbsp;
<input type="submit" value="刪除選中" class="btn" onclick="if(confirm('確定要刪除選中分站嗎？此操作將不可撤銷')){this.form.action='?file=<?php echo $file;?>&action=delete'}else{return false;}"/>&nbsp;
