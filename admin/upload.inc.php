...
$menus = array (
    array('上傳記錄', '?file='.$file),
    array('選擇分表', '?file='.$file.'&action=part'),
);
...
switch($action) {
	...
	break;
	default:
		$sfields = array('按條件', '文件名', '會員', '來源', '後綴', '信息ID');
		$dfields = array('fileurl', 'fileurl', 'username', 'upfrom', 'fileext', 'itemid');
		$sorder  = array('排序方式', '文件大小降序', '文件大小升序', '上傳時間降序', '上傳時間升序', '圖片寬度降序', '圖片寬度升序', '圖片高度降序', '圖片高度升序');
