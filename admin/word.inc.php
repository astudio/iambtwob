...
$menus = array (
    array('編輯助手', '?file='.$file),
);
switch($action) {
	case 'upload':
		$_FILES['uploadfile']['size'] or dalert('請選擇zip文件');
		...
		if($upload->save()) {
			...
			if($F) {
				...
				if($htm) {
					...
					dalert('系統未在壓縮包內找到HTM文件');
				}
			} else {
				dalert('解壓縮失敗，請檢查目錄權限');
			}
		} else {
	...
	case 'run':
		...
		echo '<html>';
		echo '<meta http-equiv="Content-Type" content="text/html;charset=utf-8'.DT_CHARSET.'"/>';
		echo '<body>';
		echo $code;
		echo '</body>';
		echo '</html>';
