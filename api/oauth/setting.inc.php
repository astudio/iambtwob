<?php
defined('IN_DESTOON') or exit('Access Denied');
?>
<table cellpadding="2" cellspacing="1" class="tb">

<tr>
<td class="tl">QQ登錄</td>
<td>
<input type="radio" name="oauth[qq][enable]" value="1"  <?php if($qq['enable']) echo 'checked';?> onclick="Dd('oa_qq').style.display='';"/> 啟用&nbsp;&nbsp;&nbsp;&nbsp;
<input type="radio" name="oauth[qq][enable]" value="0"  <?php if(!$qq['enable']) echo 'checked';?> onclick="Dd('oa_qq').style.display='none';"/> 禁用&nbsp;&nbsp;&nbsp;&nbsp;
<img src="<?php echo DT_PATH;?>api/oauth/qq/ico.png" align="absmiddle"/> <a href="<?php echo $MODULE[3]['linkurl'];?>redirect.php?url=http://connect.opensns.qq.com/" target="_blank" class="t">帳號申請</a>
</td>
</tr>
<tbody style="display:<?php echo $qq['enable'] ? '' : 'none';?>" id="oa_qq">
<tr>
<td class="tl">顯示名稱</td>
<td><input type="text" size="30" name="oauth[qq][name]" value="<?php echo $qq['name'];?>"/></td>
</tr>
<tr>
<td class="tl">顯示順序</td>
<td><input type="text" size="2" name="oauth[qq][order]" value="<?php echo $qq['order'];?>"/></td>
</tr>
<tr>
<td class="tl">APP ID</td>
<td><input type="text" size="40" name="oauth[qq][id]" value="<?php echo $qq['id'];?>"/></td>
</tr>
<tr>
<td class="tl">APP KEY</td>
<td><input type="text" size="40" name="oauth[qq][key]" value="<?php echo $qq['key'];?>"/></td>
</tr>
</tbody>

<tr>
<td class="tl">新浪微博</td>
<td>
<input type="radio" name="oauth[sina][enable]" value="1"  <?php if($sina['enable']) echo 'checked';?> onclick="Dd('oa_sina').style.display='';"/> 啟用&nbsp;&nbsp;&nbsp;&nbsp;
<input type="radio" name="oauth[sina][enable]" value="0"  <?php if(!$sina['enable']) echo 'checked';?> onclick="Dd('oa_sina').style.display='none';"/> 禁用&nbsp;&nbsp;&nbsp;&nbsp;
<img src="<?php echo DT_PATH;?>api/oauth/sina/ico.png" align="absmiddle"/> <a href="<?php echo $MODULE[3]['linkurl'];?>redirect.php?url=http://open.t.sina.com.cn/" target="_blank" class="t">帳號申請</a>
</td>
</tr>
<tbody style="display:<?php echo $sina['enable'] ? '' : 'none';?>" id="oa_sina">
<tr>
<td class="tl">顯示名稱</td>
<td><input type="text" size="30" name="oauth[sina][name]" value="<?php echo $sina['name'];?>"/></td>
</tr>
<tr>
<td class="tl">顯示順序</td>
<td><input type="text" size="2" name="oauth[sina][order]" value="<?php echo $sina['order'];?>"/></td>
</tr>
<tr>
<td class="tl">App Key</td>
<td><input type="text" size="40" name="oauth[sina][id]" value="<?php echo $sina['id'];?>"/></td>
</tr>
<tr>
<td class="tl">App Secret</td>
<td><input type="text" size="40" name="oauth[sina][key]" value="<?php echo $sina['key'];?>"/></td>
</tr>
</tbody>

<tr>
<td class="tl">百度登錄</td>
<td>
<input type="radio" name="oauth[baidu][enable]" value="1"  <?php if($baidu['enable']) echo 'checked';?> onclick="Dd('oa_baidu').style.display='';"/> 啟用&nbsp;&nbsp;&nbsp;&nbsp;
<input type="radio" name="oauth[baidu][enable]" value="0"  <?php if(!$baidu['enable']) echo 'checked';?> onclick="Dd('oa_baidu').style.display='none';"/> 禁用&nbsp;&nbsp;&nbsp;&nbsp;
<img src="<?php echo DT_PATH;?>api/oauth/baidu/ico.png" align="absmiddle"/> <a href="<?php echo $MODULE[3]['linkurl'];?>redirect.php?url=http://dev.baidu.com/connect/" target="_blank" class="t">帳號申請</a>
</td>
</tr>
<tbody style="display:<?php echo $baidu['enable'] ? '' : 'none';?>" id="oa_baidu">
<tr>
<td class="tl">顯示名稱</td>
<td><input type="text" size="30" name="oauth[baidu][name]" value="<?php echo $baidu['name'];?>"/></td>
</tr>
<tr>
<td class="tl">顯示順序</td>
<td><input type="text" size="2" name="oauth[baidu][order]" value="<?php echo $baidu['order'];?>"/></td>
</tr>
<tr>
<td class="tl">API Key</td>
<td><input type="text" size="40" name="oauth[baidu][id]" value="<?php echo $baidu['id'];?>"/></td>
</tr>
<tr>
<td class="tl">Secret Key</td>
<td><input type="text" size="40" name="oauth[baidu][key]" value="<?php echo $baidu['key'];?>"/></td>
</tr>
</tbody>

<tr>
<td class="tl">MSN</td>
<td>
<input type="radio" name="oauth[msn][enable]" value="1"  <?php if($msn['enable']) echo 'checked';?> onclick="Dd('oa_msn').style.display='';"/> 啟用&nbsp;&nbsp;&nbsp;&nbsp;
<input type="radio" name="oauth[msn][enable]" value="0"  <?php if(!$msn['enable']) echo 'checked';?> onclick="Dd('oa_msn').style.display='none';"/> 禁用&nbsp;&nbsp;&nbsp;&nbsp;
<img src="<?php echo DT_PATH;?>api/oauth/msn/ico.png" align="absmiddle"/> <a href="<?php echo $MODULE[3]['linkurl'];?>redirect.php?url=http://manage.dev.live.com/" target="_blank" class="t">帳號申請</a>
</td>
</tr>
<tbody style="display:<?php echo $msn['enable'] ? '' : 'none';?>" id="oa_msn">
<tr>
<td class="tl">顯示名稱</td>
<td><input type="text" size="30" name="oauth[msn][name]" value="<?php echo $msn['name'];?>"/></td>
</tr>
<tr>
<td class="tl">顯示順序</td>
<td><input type="text" size="2" name="oauth[msn][order]" value="<?php echo $msn['order'];?>"/></td>
</tr>
<tr>
<td class="tl">Client ID</td>
<td><input type="text" size="40" name="oauth[msn][id]" value="<?php echo $msn['id'];?>"/></td>
</tr>
<tr>
<td class="tl">Client secret</td>
<td><input type="text" size="40" name="oauth[msn][key]" value="<?php echo $msn['key'];?>"/></td>
</tr>
</tbody>
</table>