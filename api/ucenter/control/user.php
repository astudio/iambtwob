...
class usercontrol extends base {
	...
	function onregbbs() {//DT ADD
		$this->init_input();
		$uid = $this->input('uid');
		$username = $this->input('username');
		$password =  md5(md5($this->input('password')));
		$email = $this->input('email');
		$pre = UC_BBSPRE;
		$this->db->query("INSERT INTO {$pre}common_member (uid,email,username,password,groupid,regdate,timeoffset) VALUES ('$uid','$email','$username','$password','10','{$this->time}','9999')");
		$this->db->query("INSERT INTO {$pre}common_member_count (uid) VALUES ('$uid')");
		$this->db->query("INSERT INTO {$pre}common_member_field_forum (uid,customshow) VALUES ('$uid','26')");
		$this->db->query("INSERT INTO {$pre}common_member_field_home (uid) VALUES ('$uid')");
		$this->db->query("INSERT INTO {$pre}common_member_profile (uid) VALUES ('$uid')");
		$this->db->query("INSERT INTO {$pre}common_member_status (uid,regip,lastip,lastvisit,lastactivity) VALUES ('$uid','{$this->onlineip}','{$this->onlineip}','{$this->time}','{$this->time}')");
		$this->db->query("INSERT INTO {$pre}common_onlinetime (uid,lastupdate) VALUES ('$uid','{$this->time}')");
		return $uid;
	}
