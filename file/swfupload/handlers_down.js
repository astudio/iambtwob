...
function uploadSuccess(file, serverData) {
	try {
		...
		if (serverData.substring(0, 7) === "FILEID:") {
			Dd('fileurl').value = serverData.substring(7);
			Dd('tdPercentUploaded').innerHTML = '100%';
			auto_type();
			file_size = (Dd('tdSizeUploaded').innerHTML);
			if(file_size) {
			...
			if(serverData.indexOf('Kb)') != -1) {
				Dd('tdPercentUploaded').innerHTML = '上傳失敗，文件過大';
			} else if(serverData.indexOf('Denied') != -1) {
				Dd('tdPercentUploaded').innerHTML = '上傳失敗，未知錯誤';
			} else {
				Dd('tdPercentUploaded').innerHTML = '上傳失敗，請重試';
				//alert('上傳失敗'+serverData);
			}
		}
