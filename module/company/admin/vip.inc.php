...
$menus = array (
    ...
    array('過期'.VIP, '?moduleid='.$moduleid.'&file='.$file.'&action=expire'),
...
switch($action) {
		...
		if($submit) {		
			...
			foreach($usernames as $username) {
				...
				if($money) {
					...
					money_record($username, $money, '站內', $_username, $reason, $GROUP[$vip['groupid']]['groupname']);
				}
				if($credit) {
	...
	break;
	default:
		$sfields = array('按條件', '公司名', '會員名');
		$dfields = array('keyword', 'company', 'username');
		$sorder  = array('結果排序方式', '服務開始降序', '服務開始升序', '服務結束降序', '服務結束升序', VIP.'指數降序', VIP.'指數升序', '理論值降序', '理論值升序', '修正值降序', '修正值升序', '會員ID降序', '會員ID升序');
