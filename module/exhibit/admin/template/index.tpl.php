...
<select name="process">
<option value="0"<?php if($process == 0) echo ' selected';?>>狀態</option>
<option value="1"<?php if($process == 1) echo ' selected';?>>未開始</option>
<option value="2"<?php if($process == 2) echo ' selected';?>>進行中</option>
<option value="3"<?php if($process == 3) echo ' selected';?>>已過期</option>
</select>&nbsp;
...
<option value="fromtime" <?php if($datetype == 'fromtime') echo 'selected';?>>開始日期</option>
<option value="totime" <?php if($datetype == 'totime') echo 'selected';?>>結束日期</option>
...
<?php echo category_select('catid', '不限分類', $catid, $moduleid);?> &nbsp;
<?php echo ajax_area_select('areaid', '不限地區', $areaid);?>&nbsp;
...
<td><a href="<?php echo $v['caturl'];?>" target="_blank"><?php echo $v['catname'];?></a></td><td><?php if($v['level']) {?><a href="?moduleid=<?php echo $moduleid;?>&file=<?php echo $file;?>&action=<?php echo $action;?>&level=<?php echo $v['level'];?>"><img src="admin/image/level_<?php echo $v['level'];?>.gif" title="<?php echo $v['level'];?>級" alt=""/></a><?php } ?></td>
<td align="left">&nbsp;<a href="<?php echo $v['linkurl'];?>" target="_blank"><?php echo $v['title'];?></a><?php if($v['thumb']) {?> <a href="javascript:_preview('<?php echo $v['thumb'];?>');"><img src="admin/image/img.gif" width="10" height="10" title="標題圖,點擊預覽" alt=""/></a><?php } ?></td>
...
<span class="f_red f_r">批量刪除過期 <input type="text" size="3" name="days" id="days" value="60"/> 
天的<?php echo $MOD['name'];?> <input type="submit" value="刪 除" class="btn" onclick="if(Dd('days').value==''){alert('請填寫天數');return false;}if(confirm('確定要刪除過期'+Dd('days').value+'天的<?php echo $MOD['name'];?>嗎？此操作將不可撤銷')){this.form.action='?moduleid=<?php echo $moduleid;?>&file=<?php echo $file;?>&action=expire&refresh=1&delete=1'}else{return false;}"/></span>
<input type="submit" value=" 回收站 " class="btn" onclick="this.form.action='?moduleid=<?php echo $moduleid;?>&file=<?php echo $file;?>&action=delete&recycle=1';"/>&nbsp;
<input type="submit" value=" 徹底刪除 " class="btn" onclick="if(confirm('確定要刪除選中<?php echo $MOD['name'];?>嗎？此操作將不可撤銷')){this.form.action='?moduleid=<?php echo $moduleid;?>&file=<?php echo $file;?>&action=delete'}else{return false;}"/>
