...
isset($aid) or $aid = 0;
$menus = array (
    array('添加廣告位', '?moduleid='.$moduleid.'&file='.$file.'&action=add_place'),
    array('廣告位管理', '?moduleid='.$moduleid.'&file='.$file),
    array('廣告管理', '?moduleid='.$moduleid.'&file='.$file.'&action=list'),
    array('廣告審核', '?moduleid='.$moduleid.'&file='.$file.'&action=list&job=check'),
    array('更新廣告', '?moduleid='.$moduleid.'&file='.$file.'&action=html'),
);
$do = new ad();
$do->pid = $pid;
$do->aid = $aid;
...
switch($action) {
	...
	case 'list':
		$job = isset($job) ? $job : '';
		$P = $do->get_place();
		$sfields = array('按條件', '廣告名稱', '廣告介紹', '會員名');
		$dfields = array('title', 'title', 'introduce', 'username');
		$sorder  = array('結果排序方式', '添加時間降序', '添加時間升序', '開始時間降序', '開始時間升序', '結束時間降序', '結束時間升序', '瀏覽次數降序', '瀏覽次數升序');
		...
		$type_select  = dselect($TYPE, 'typeid', '廣告類型', $typeid);
	...
	case 'view':
		...
		if($pid) {
			$p = $do->get_one_place();
			$head_title = '廣告位 ['.$p['name'].'] 預覽';
			$typeid = $p['typeid'];
		} else if($aid) {
			$a = $do->get_one();
			$head_title = '廣告 ['.$a['title'].'] 預覽';
			$pid = $a['pid'];
			$typeid = $a['typeid'];
			if($typeid > 5) {
	...
	case 'html':
		...
		msg('ID從'.$fid.'至'.($aid-1).'[廣告]生成成功'.progress($sid, $fid, $tid), "?moduleid=$_moduleid&file=$file&action=$action&sid=$sid&fid=$aid&tid=$tid&num=$num&all=$all&one=$one");
	break;
	default:
		isset($typeid) or $typeid = 0;
