...
$menus = array (
    ...
    array('生成網頁', '?moduleid='.$moduleid.'&file='.$file.'&action=html'),
    array('公告分類', '?file=type&item=announce'),
);
...
switch($action) {
	...
	case 'html':
		...
		msg('ID從'.$fid.'至'.($itemid-1).'[公告]生成成功'.progress($sid, $fid, $tid), "?moduleid=$moduleid&file=$file&action=$action&sid=$sid&fid=$itemid&tid=$tid&num=$num&all=$all&one=$one");
	break;
	case 'delete':
		$itemid or msg('請選擇公告');
		$do->delete($itemid);
	...
	break;
	default:
		$sorder  = array('結果排序方式', '添加時間降序', '添加時間升序', '瀏覽次數降序', '瀏覽次數升序', '開始時間降序', '開始時間升序', '到期時間降序', '到期時間升序');
		...
		$type_select = type_select('announce', 1, 'typeid', '請選擇分類', $typeid);
