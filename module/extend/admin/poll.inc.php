...
$do = new poll();
$menus = array (
    array('添加票選', '?moduleid='.$moduleid.'&file='.$file.'&action=add'),
    array('票選列表', '?moduleid='.$moduleid.'&file='.$file),
    array('更新地址', '?moduleid='.$moduleid.'&file='.$file.'&action=update'),
    array('票選分類', '?file=type&item=poll'),
);
...
switch($action) {
	...
	break;
	default:
		$sorder  = array('結果排序方式', '添加時間降序', '添加時間升序', '投票總數降序', '投票總數升序', '瀏覽次數降序', '瀏覽次數升序', '選項總數降序', '選項總數升序', '開始時間降序', '開始時間升序', '到期時間降序', '到期時間升序');
		...
		$type_select = type_select('poll', 1, 'typeid', '請選擇分類', $typeid);
		$order_select  = dselect($sorder, 'order', '', $order);
		$condition = '1';
