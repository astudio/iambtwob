...
$menus = array (
    ...
    array('排名審核', '?moduleid='.$moduleid.'&file='.$file.'&action=check'),
    array('起價設置', '?moduleid='.$moduleid.'&file='.$file.'&action=price'),
    array('生成排名', '?moduleid='.$moduleid.'&file='.$file.'&action=html'),
    array('排名設置', '?moduleid='.$moduleid.'&file=setting'),
);
$CATEGORY = array();
if(in_array($action, array('', 'check'))) {
	$sfields = array('關鍵詞', '會員名', '公司名', '信息ID', '價格');
	$dfields = array('word', 'username', 'company', 'tid', 'price');
	$sorder  = array('結果排序方式', '價格降序', '價格升序', '添加時間降序', '添加時間升序', '開始時間降序', '開始時間升序', '到期時間降序', '到期時間升序');
...
switch($action) {
	...
	case 'html':
		...
		msg('ID從'.$fid.'至'.($itemid-1).'[排名推廣]生成成功'.progress($sid, $fid, $tid), "?moduleid=$moduleid&file=$file&action=$action&sid=$sid&fid=$itemid&tid=$tid&num=$num&all=$all&one=$one");
	break;
	case 'price':
		if($submit) {
