...
$menus = array (
    array('生成網頁', '?moduleid='.$moduleid.'&file='.$file),
    array('一鍵生成', '?moduleid='.$moduleid.'&file='.$file.'&action=all'),
    array('模塊首頁', $MOD['linkurl'], ' target="_blank"'),
);
...
switch($action) {
	...
	case 'index':
		tohtml('index', $module);
		$all ? msg($MOD['name'].'首頁生成成功', '?moduleid='.$moduleid.'&file='.$file.'&action=list&all=1&one='.$one) : dmsg($MOD['name'].'首頁生成成功', $this_forward);
	break;
	case 'list':
		if(!$MOD['list_html']) {
		...
		if(isset($catids)) {
			...
			if(isset($KEYS[$catids])) {
				...
				if($fpage <= $total) {
					...
					tohtml('list', $module);
					$fid = $bfid;
					msg($MOD['name'].' ['.$CATEGORY[$bcatid]['catname'].'] 第'.$fpage.'頁至第'.($fpage+$num-1).'頁生成成功'.progress($sid, $fid, $tid), '?moduleid='.$moduleid.'&file='.$file.'&action='.$action.'&catids='.$catids.'&tid='.$tid.'&all='.$all.'&one='.$one.'&fpage='.($fpage+$num));
				}
	...
	case 'show':
		...
		msg('ID從'.$fid.'至'.($itemid-1).$MOD['name'].($update ? '更新' : '生成').'成功'.progress($sid, $fid, $tid), "?moduleid=$moduleid&file=$file&action=$action&sid=$sid&fid=$itemid&tid=$tid&num=$num&update=$update&all=$all&one=$one");
	break;
	case 'update_resume':
		$table = $DT_PRE.'resume';
		$catid = isset($catid) ? intval($catid) : '';
		...
		msg('ID從'.$fid.'至'.($itemid-1).'簡歷更新成功', "?moduleid=$moduleid&file=$file&action=$action&fid=$itemid&tid=$tid&num=$num");
	break;
	case 'cate':
		$catid or msg('請選擇分類');
		isset($num) or $num = 50;
		...
		if($fid <= $total) {
			tohtml('list', $module);
			msg('第'.$fid.'頁至第'.($fid+$num-1).'頁生成成功', '?moduleid='.$moduleid.'&file='.$file.'&action='.$action.'&catid='.$catid.'&fid='.($fid+$num).'&num='.$num.'&fpage='.$fpage.'&tpage='.$tpage);
		} else {
			dmsg('生成成功', $this_forward);
		}
	break;
