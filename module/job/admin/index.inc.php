...
$menus = array (
    ...
    array('過期招聘', '?moduleid='.$moduleid.'&action=expire'),
    array('未通過招聘', '?moduleid='.$moduleid.'&action=reject'),
    array('回收站', '?moduleid='.$moduleid.'&action=recycle'),
    array('移動招聘', '?moduleid='.$moduleid.'&action=move'),
);
...
if(in_array($action, array('', 'check', 'expire', 'reject', 'recycle'))) {
	...
	$MARRIAGE[0] = '婚姻狀況';
	$EDUCATION[0] = '學歷要求';
	$sfields = array('模糊', '職位名稱', '簡介', '招聘部門', '公司名', '聯繫人', '聯繫電話', '聯繫地址', '電子郵件', '聯繫MSN', '聯繫QQ', '會員名', 'IP');
	$dfields = array('keyword', 'title', 'introduce', 'department', 'company', 'truename', 'telephone', 'address', 'email', 'msn', 'qq','username', 'ip');
	$sorder  = array('結果排序方式', '更新時間降序', '更新時間升序', '添加時間降序', '添加時間升序', '瀏覽次數降序', '瀏覽次數升序', '招聘人數降序', '招聘人數升序', '最低待遇降序', '最低待遇升序', '最高待遇降序', '最高待遇升序', '學歷高低降序', '學歷高低升序', '信息ID降序', '信息ID升序');
