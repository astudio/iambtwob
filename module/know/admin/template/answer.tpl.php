...
提問ID <input type="text" size="5" name="qid" value="<?php echo $qid;?>"/>&nbsp;
...
票數 (<?php echo $v['vote'];?>) &nbsp;|&nbsp;<a href="<?php echo $MODULE[3]['linkurl'];?>redirect.php?mid=<?php echo $moduleid;?>&itemid=<?php echo $v['qid'];?>" target="_blank">原文</a>
</span>
...
<div class="b5 c_b"> </div>
<div><strong>IP:</strong><a href="?moduleid=<?php echo $moduleid;?>&file=<?php echo $file;?>&action=<?php echo $action;?>&ip=<?php echo $v['ip'];?>"><?php echo $v['ip'];?></a> <span class="f_dblue">來自:<?php echo ip2area($v['ip']);?></span></div>

<?php if($v['linkurl']) { ?>
<div class="b5 c_b"> </div>
<div>參考資料：<a href="<?php echo $MODULE[3]['linkurl'];?>redirect.php?url=<?php echo urlencode($v['linkurl']);?>" target="_blank"><?php echo $v['linkurl'];?></a></div>
<?php } ?>
</td>
<td>
...
<div class="btns">
<input type="submit" value=" 刪 除 " class="btn" onclick="if(confirm('確定要刪除選中答案嗎？此操作將不可撤銷')){this.form.action='?moduleid=<?php echo $moduleid;?>&file=<?php echo $file;?>&action=delete'}else{return false;}"/>&nbsp;
<?php if($action == 'check') { ?>
