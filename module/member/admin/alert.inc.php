...
$do = new alert();
$menus = array (
    array('發送商機', '?moduleid='.$moduleid.'&file='.$file.'&action=send'),
    array('貿易提醒', '?moduleid='.$moduleid.'&file='.$file),
    array('審核提醒', '?moduleid='.$moduleid.'&file='.$file.'&action=check'),
);
$mids = array();
...
if(in_array($action, array('', 'check'))) {
	$sfields = array('按條件', '關鍵詞', '會員名', 'Email');
	$dfields = array('word', 'word', 'username', 'email');
	$sorder  = array('結果排序方式', '添加時間降序', '添加時間升序', '發送時間降序', '發送時間升序', '發送頻率降序', '發送頻率升序');
	...
	case 'send':
		if(isset($send)) {
			...
			if($fid <= $tid) {
				...
				dmsg('發送成功', "?moduleid=$moduleid&file=$file");
			}
			msg('ID從'.$fid.'至'.($itemid-1).'發送成功'.progress($sid, $fid, $tid), "?moduleid=$moduleid&file=$file&action=$action&sid=$sid&fid=$itemid&tid=$tid&num=$num&send=1");
		} else {
			$item = cache_read('alert.php');
			if($item) {
				extract($item);
			} else {
				$title = $DT['sitename'].'[貿易提醒]';
				$total = 30;
				$num = 5;
				$template = '';
				$sql = 'AND vip>0';
