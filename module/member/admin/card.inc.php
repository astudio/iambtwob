...
switch($action) {
	case 'add':
		if($submit) {
			...
			if($amount <= 0) msg('面額格式錯誤');
			$prefix_length = strlen($prefix);
			$number_length = intval($number_length);
			if($number_length < 8) msg('卡號不能少於8位');
			$rand_length = $number_length - $prefix_length;
			if($rand_length < 4)  msg('卡號長度和前綴長度差不能少於4位');
			$password_length = intval($password_length);
			if($password_length < 6) msg('密碼不能少於6位');
			$number_part = trim($number_part);
			if(!preg_match("/^[0-9a-zA-z]{6,}$/", $number_part)) msg('卡號只能由6位以上數字和字母組成');
	...
	default:
		...
		$sfields = array('按條件', '卡號', '密碼', '面額', '會員', 'IP', '操作人');
		$dfields = array('number', 'number', 'password', 'amount', 'username', 'ip', 'editor');
		$sorder  = array('排序方式', '面額降序', '面額升序', '充值時間降序', '充值時間升序', '到期時間降序', '到期時間升序', '制卡時間降序', '制卡時間升序');
