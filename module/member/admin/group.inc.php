...
$menus = array (
    array('會員組添加', '?moduleid='.$moduleid.'&file='.$file.'&action=add'),
    array('會員組管理', '?moduleid='.$moduleid.'&file='.$file),
);
$do = new group;
...
if($action == 'add') {
	if($submit) {
		if(!$groupname) msg('會員組名稱不能為空');
		if($setting['fee_mode']) {//收費會員
