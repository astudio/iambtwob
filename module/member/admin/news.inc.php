...
$do = new news();
$menus = array (
    array('添加新聞', '?moduleid='.$moduleid.'&file='.$file.'&action=add'),
    array('新聞列表', '?moduleid='.$moduleid.'&file='.$file),
    array('審核新聞', '?moduleid='.$moduleid.'&file='.$file.'&action=check'),
    array('未通過新聞', '?moduleid='.$moduleid.'&file='.$file.'&action=reject'),
...
switch($action) {
	...
	case 'push':
		...
		if($submit) {
			...
			dmsg('推送成功'.$i.'條新聞', "?moduleid=$moduleid&file=$file");
		} else {
			$itemid or msg('請選擇新聞');
			$ids = implode(',', $itemid);
