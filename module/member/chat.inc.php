...
switch($action) {
	...
	case 'del':
		...
		dmsg('刪除成功', 'chat.php?tab='.(isset($tab) ? $tab : 1));
	break;
	case 'off':
		$chatid or exit;
		$chat = $db->get_one("SELECT * FROM {$DT_PRE}chat WHERE chatid='$chatid'");
	...
	case 'down':
		if($data) {
			...
			$data = '<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd"><html xmlns="http://www.w3.org/1999/xhtml"><head><meta http-equiv="Content-Type" content="text/html;charset=utf-8'.DT_CHARSET.'"/><title>聊天記錄</title><style type="text/css">'.$css.'</style><base href="'.$MOD['linkurl'].'"/></head><body><div id="chat">'.$data.'</div></body></html>';
	...
	default:
		...
		if(isset($touser) && check_name($touser)) {
			if($touser == $_username) dalert('不能與自己對話', 'chat.php');
			if(!$MG['chat']) {
				login();
				dalert('您所在的會員組沒有權限發起對話', 'grade.php');
			}
			$user = userinfo($touser);
			$user or dalert('會員不存在', 'chat.php');
			...
			if($online == 1) {
				...
				$head_title = '與【'.$user['company'].'】對話中';
				$chat = $db->get_one("SELECT * FROM {$DT_PRE}chat WHERE chatid='$chatid'");
				if($chat) {
					if($chat['status'] == 4) {//超時後重新連接
						$chat['status'] = 2;
			...
			if($chat && $chat['touser'] == $_username) {
				...
				$head_title = '與'.($user['type'] == 'guest' ? '【遊客】' : $chat['fromuser']).'對話中';
			} else {
				dheader('chat.php');
			}
			$type = 2;
