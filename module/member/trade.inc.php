...
if($action == 'update') {
	...
	switch($step) {
		case 'edit_price'://修改價格||確認訂單
			if($td['status'] != 0 || $td['seller'] != $_username) message($L['trade_msg_deny']);
			if($DT['trade'] && $_trade == '') message('系統採用了'.$DT['trade_nm'].'擔保交易，請先綁定您的'.$DT['trade_nm'].'帳號', '?action=bind');
			if($submit) {
				$fee = dround($fee);
		...
		case 'comment'://交易評價
			...
			if($_username == $td['seller']) {
				if($td['buyer_star']) message('您已經評價過此交易');
				if($submit) {
					...
					message('評價提交成功', $forward);
				}
			} else if($_username == $td['buyer']) {
				if($td['seller_star']) message('您已經評價過此交易');
				if($submit) {
	...
	$DT['trade'] or message('系統未開啟擔保交易接口');
	$member = $db->get_one("SELECT trade,vtrade FROM {$DT_PRE}member WHERE userid=$_userid");
	if($submit) {
		if($member['trade'] && $member['vtrade']) message('您的帳號已經綁定，不可再修改<br/>如果需要修改，請與網站聯繫');
		if($trade) {
			if($DT['trade'] == 'alipay' && !is_email($trade) && !is_mobile($trade)) message($DT['trade_nm'].'帳號格式不正確');
			$r = $db->get_one("SELECT userid FROM {$DT_PRE}member WHERE trade='$trade' AND vtrade=1");
			if($r) message('帳號綁定已經存在，請檢查您的帳號');
		} else {
			$trade = '';
		}
