...
if(in_array($action, array('', 'check', 'reject', 'recycle'))) {
	$sfields = array('模糊', '標題', '簡介', '會員名', 'IP');
	$dfields = array('keyword', 'title', 'introduce', 'username', 'ip');
	$sorder  = array('結果排序方式', '添加時間降序', '添加時間升序', '更新時間降序', '更新時間升序', '瀏覽次數降序', '瀏覽次數升序', '信息ID降序', '信息ID升序');
...
switch($action) {
	...
	case 'dir':
		...
		preg_match("/[a-z0-9_\-]/i", $name) or msg('目錄名應為數字、字母、下劃線、中劃線的組合');
		$dir = DT_ROOT.'/file/temp/'.$name;
		is_dir($dir) or msg('目錄不存在');
		...
		dir_delete($dir);
		msg('成功添加<strong> '.$i.' </strong>張圖片', '?moduleid='.$moduleid.'&action=item&itemid='.$itemid);
	break;
	case 'zip':
		$itemid or msg();
		$_FILES['uploadfile']['size'] or msg('請選擇zip文件');
		...
		if($upload->save()) {
			...
			if(glob(DT_ROOT.'/file/temp/'.$name.'/*.*')) {
				msg('上傳並解壓縮成功，正在讀取..', '?moduleid='.$moduleid.'&action=dir&name='.$name.'&itemid='.$itemid);
			} else {
				msg('解壓縮失敗，請檢查目錄權限');
			}
		} else {
